<script>
  ...
  function isSpreadable({ path }) {
    ...
    let targetArr = _.get(data, targetArrPath);
    // the target element
    const target = _.last(path);

    if (targetArrPath.length === 0 && _.isArray(data)) {
      targetArr = data;
    }
  ...
  function spreadHandler({ path }) {
    ...
    if (isTargetValid(targetObj[target])) {
      ...
      // if the element is the root element then just set the
      // then instead of updating the value in data using a path just replace the whole
      // data variable with updatedArr
      if (targetArrPath.length === 0 && _.isArray(data)) data = updatedArr;
